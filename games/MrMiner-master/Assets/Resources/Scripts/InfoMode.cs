using System;
using System.Linq;
using UnityEngine;
using UnityEngine.EventSystems;

public class InfoMode : MonoBehaviour, IPointerDownHandler, IPointerClickHandler,
    IPointerUpHandler
{
    [NonSerialized] public bool Mode;
    public AnimationCurve onPressCurve;

    private static AudioClip _mouseDownAudioClip;
    private static AudioClip _mouseUpAudioClip;
    private static AudioClip _infoEnabled;
    private AudioSource _audioSource;
    private bool _onDownAnimation, _onUpAnimation;
    private float _timeStart;
    private float _shopBaseInitialLocalPosY;
    private DataStorage _dataStorage;
    private Transform _child;

    private void Start()
    {
        _dataStorage = GameObject.FindGameObjectWithTag("DataStorage").GetComponent<DataStorage>();
        _mouseDownAudioClip = Resources.Load("Raws/click_down_003") as AudioClip;
        _mouseUpAudioClip = Resources.Load("Raws/click_up_002") as AudioClip;
        _infoEnabled = Resources.Load("Raws/bleep_01") as AudioClip;
        _audioSource = GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Camera>()
            .GetComponent<AudioSource>();
        _child = transform.GetChild(1);
        _shopBaseInitialLocalPosY = _child.transform.localPosition.y;
    }

    private void Update()
    {
        if (!_onDownAnimation && !_onUpAnimation)
            return;

        var t = Time.time - _timeStart;
        float localY = default;
        if (_onDownAnimation)
            localY = -15f * onPressCurve.Evaluate(t * 1 / 0.06f);
        else if (_onUpAnimation)
            localY = -15f + 15f * onPressCurve.Evaluate(t * 1 / 0.06f);

        _child.transform.localPosition = new Vector2(0, _shopBaseInitialLocalPosY + localY);
    }

    public void OnPointerClick(PointerEventData eventData)
    {
        var trigger = Mode ? "CloseInfo" : "OpenInfo";
        foreach (var animator in _dataStorage.user.ShopItemBuildingAnimators)
        {
            animator.enabled = true;
            animator.SetTrigger(trigger);
        }
        foreach (var animator in _dataStorage.user.ShopItemStoreAnimators)
        {
            animator.enabled = true;
            animator.SetTrigger(trigger);
        }
        if (!Mode)
            _audioSource.PlayOneShot(_infoEnabled);
        Mode = !Mode;
    }

    public void OnPointerDown(PointerEventData eventData)
    {
        _audioSource.PlayOneShot(_mouseDownAudioClip);
        _timeStart = Time.time;
        _onDownAnimation = true;
        _onUpAnimation = false;
    }

    public void OnPointerUp(PointerEventData eventData)
    {
        _audioSource.PlayOneShot(_mouseUpAudioClip);
        _timeStart = Time.time;
        _onDownAnimation = false;
        _onUpAnimation = true;
    }
}